...
from .config import FLEET_LABEL, ContainersConfig, FleetConfig
...
def _devcontainer_up(fleet_name: str, fleet: FleetConfig, name: str) -> str:
    ...
    devcontainer applies as a docker label) and renames it to ``name``.
    """
...
def up(config: ContainersConfig, fleet_name: str, count: int | None = None) -> list[str]:
    ...
    for idx in indices:
        ...
        if fleet.devcontainer_path:
            created.append(_devcontainer_up(fleet_name, fleet, name))
        else:
