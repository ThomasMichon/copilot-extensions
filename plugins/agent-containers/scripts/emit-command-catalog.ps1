...
$specs = '[{"id":"agent-containers","relativePath":"bin\\agent-containers.ps1","purpose":"Manage local container fleets leases and dispatch"}]' | ConvertFrom-Json
$commands = @(
    ...
    if ($hookInput) {
        try {
            ...
            [Console]::Error.WriteLine(
                '[agent-containers] invalid sessionStart payload; catalog deduplication disabled: ' +
                $_.Exception.Message
            )
        }
    }
...
try {
    ...
    [Console]::Error.WriteLine(
        '[agent-containers] catalog deduplication launch identity unavailable: ' +
        $_.Exception.Message
    )
}
...
if ($sessionId -and $launchToken) {
    ...
    try {
        ...
        $markerPath = ''
        [Console]::Error.WriteLine(
            '[agent-containers] catalog deduplication unavailable: ' +
            $_.Exception.Message
        )
    } catch {
        $markerPath = ''
