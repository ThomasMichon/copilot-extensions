...
command_specs='[{"id":"agent-dispatch","relativePath":"bin/agent-dispatch","purpose":"Coordinate queued agent work and task lifecycles"}]'
hook_input=""
[ -t 0 ] || hook_input="$(cat)"
...
if raw_hook_input:
    try:
        ...
        print(
            f"[agent-dispatch] invalid sessionStart payload; "
...
if session_id:
    ...
    try:
        ...
        marker_path = ""
        print(
            f"[agent-dispatch] catalog deduplication unavailable: {error}",
            file=sys.stderr,
        )
