"""Tests for the coordinator generation self-retire decision.

Exercises ``agent_dispatch.self_retire.is_superseded`` -- the fail-safe predicate
a demoted coordinator uses to decide it has been superseded by a live,
strictly-newer generation. It must say "yes" in exactly one shape and "no" (stay
alive) for every ambiguous state, so the matrix is exhaustive on the "no" side.
"""
...
from agent_dispatch.self_retire import _is_listening, is_superseded
...
def test_not_superseded_when_table_not_a_dict():
    assert is_superseded(
        CONFIG_DIR, my_pid=111, my_generation=4,
        read_table=lambda _d: ["nope"], is_listening=_always_listening,
    ) is False
