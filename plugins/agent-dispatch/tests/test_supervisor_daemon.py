...
# -- live-update version watch (reload-exit) ---------------------------------


class _VersionSource:
    """A fake ``current-version`` reader that yields a scripted sequence.

    The last value repeats once exhausted, so a serve loop that keeps polling
    after the swap sees a stable new version (no oscillation)."""
    ...
    def __call__(self):
        self.calls += 1
        v = self._versions[min(self._i, len(self._versions) - 1)]
        self._i += 1
        return v


def test_serve_reload_exits_when_active_slot_swaps():
    """A non-elevated update swaps the active version -> the daemon drains its
    units, releases its lease, and reload-exits with RELOAD_EXIT_CODE."""
    client = FakeClient([_reg("a")])
    launcher = FakeLauncher()
    lock = FakeLock(granted=True)
    src = _VersionSource(["v1", "v2"])  # baseline v1, then v2 on the first tick
    d = _daemon(
        client, launcher, lock=lock,
        running_version="v1", active_version_source=src,
    )
    rc = d.serve()  # loops until the reload-exit (sleep is a no-op fake)
    assert rc == RELOAD_EXIT_CODE
    # It started the unit, then drained it on the reload-exit and released the lock.
    assert launcher.proc_for("a").terminated is True
    assert lock.released is True


def test_serve_no_reload_when_version_unchanged():
    client = FakeClient([_reg("a")])
    launcher = FakeLauncher()
    src = _VersionSource(["v1"])  # never diverges
    d = _daemon(
...
def test_serve_watch_disarmed_for_dev_sentinel():
    """A bare-source run (sentinel __version__) must never self-reload, even if
    the on-disk marker names some other version."""
    client = FakeClient([_reg("a")])
    launcher = FakeLauncher()
    src = _VersionSource(["v1", "v2"])
    d = _daemon(
        client, launcher, lock=FakeLock(granted=True),
        running_version="0.0.0+dev", active_version_source=src,
    )
    rc = d.serve(once=True)
    assert rc == 0
    # Disarmed: the baseline is never captured, so no marker read is even needed.
    assert d._version_baseline is None


def test_serve_watch_disarmed_without_source():
    client = FakeClient([_reg("a")])
    launcher = FakeLauncher()
    d = _daemon(
        client, launcher, lock=FakeLock(granted=True),
        running_version="v1",  # no active_version_source wired
    )
    rc = d.serve(once=True)
    assert rc == 0


def test_reload_target_ignores_marker_read_failure():
    """A transient marker read error is treated as 'no change' -- never a spurious
    reload."""
    client = FakeClient([_reg("a")])
    launcher = FakeLauncher()
    ...
    d._version_baseline = "v1"
    src.armed = True
    assert d._reload_target() is None
