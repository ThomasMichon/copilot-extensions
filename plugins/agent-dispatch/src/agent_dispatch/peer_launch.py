"""Read-only, same-cell native child boundary for dispatch's two runtime peers.
...
def launch(own_root: Path, raw_context: str, plugin: str, argv: list[str]) -> int:
    """Validate both owners, delegate peer governance/resolution, then launch."""
    if plugin not in PEERS:
        raise ValueError(f"Unsupported dispatch peer: {plugin}")
    if (
        not own_root.is_absolute() or own_root.name != "agent-dispatch"
        or own_root.parent.name != "plugins"
        or own_root.parent.parent.parent.name != "marketplaces"
    ):
        raise ValueError("Explicit dispatch context requires a canonical cell install root")
    ...
    primitive = _load_primitive(
        ...
        "_dispatch_installation_context",
    )
    declared = None
    pointer = raw_context
    ...
    own = _active_context(
        primitive, Path(pointer), durable, "agent-dispatch", cell, dict(os.environ),
    )
    if not primitive.paths_equal(own["pluginRoot"], own_root):
        raise ValueError("Explicit context does not belong to this dispatch installation")
    ...
    if (
        _active_context(primitive, Path(pointer), durable, "agent-dispatch",
                        cell, dict(os.environ)) != own
...
def main() -> int:
    try:
        if len(sys.argv) < 4:
            raise ValueError("Expected dispatch root, explicit context, and peer")
        return launch(Path(sys.argv[1]), sys.argv[2], sys.argv[3], sys.argv[4:])
    except (OSError, ValueError, ImportError, subprocess.SubprocessError) as error:
        print(f"[agent-dispatch] peer launch refused: {error}", file=sys.stderr)
        return 126
