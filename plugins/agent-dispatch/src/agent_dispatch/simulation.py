...
with a bridge one, plus ``step_*`` helpers that resolve a named transition
from the owning table and apply it through
:func:`agent_dispatch.machine_coupling.apply_transition`.
...
class World:
    """Two coupled versioned records: the task's own state (as
    ``queue.py`` would persist it) and the bridge/session's own state (as a
    session-host store would persist it), evolved independently and
    reconciled only through :mod:`agent_dispatch.machine_coupling`'s
    declared rules -- never by one directly writing the other."""
...
def apply_sequence(world: World, steps: tuple[tuple[str, str], ...]) -> World:
    ...
    where ``machine`` is ``"task"`` or ``"bridge"``. Steps whose
    precondition is not yet satisfied are silently skipped (out-of-order
    delivery) -- callers that need to assert every step actually applied
    should inspect individual :func:`step_task` / :func:`step_bridge`
    results instead."""
