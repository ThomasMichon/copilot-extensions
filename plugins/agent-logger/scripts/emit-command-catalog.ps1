    ...
    if ($hookInput) {
        try {
            ...
            [Console]::Error.WriteLine(
                '[agent-logger] invalid sessionStart payload; catalog deduplication disabled: ' +
                $_.Exception.Message
            )
        }
    }
...
try {
    ...
    [Console]::Error.WriteLine(
        '[agent-logger] catalog deduplication launch identity unavailable: ' +
        $_.Exception.Message
    )
}
...
if ($sessionId -and $launchToken) {
    ...
    try {
        ...
        $markerPath = ''
        [Console]::Error.WriteLine(
            '[agent-logger] catalog deduplication unavailable: ' +
            $_.Exception.Message
        )
    } catch {
        $markerPath = ''
