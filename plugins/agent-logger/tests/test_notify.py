...
def test_post_notify_sets_explicit_user_agent(monkeypatch):
    ...
    ok = notify_mod.post_notify("https://hub.example/hook?m={machine}", "emancipation-cube")
    ...
    assert json.loads(req.data) == {"machine": "emancipation-cube"}
    # ``{machine}`` in the URL is substituted too.
    assert req.full_url == "https://hub.example/hook?m=emancipation-cube"
...
def test_post_notify_empty_url_is_noop(monkeypatch):
    ...
    assert notify_mod.post_notify("", "emancipation-cube") is False
    assert called["n"] == 0
