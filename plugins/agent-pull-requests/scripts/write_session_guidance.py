#!/usr/bin/env python3
"""Write agent-pull-requests guidance as an exact-session side effect."""
...
_GUIDANCE_HEADER = "# Agent Pull Requests session guidance\n\n"
...
def write_session_guidance(payload: dict, *, home: Path | None = None) -> bool:
    ...
    content = (
        ...
        + "No current agent-pull-requests session guidance was available. Treat "
        "guidance as unavailable for this session-start invocation.\n"
    )
    if context and len(content.encode("utf-8")) > _GUIDANCE_MAX_BYTES:
        content = (
            _GUIDANCE_HEADER
            + "Current agent-pull-requests session guidance was omitted because it "
    ...
    try:
        ...
        if _is_link_or_reparse(instructions):
            return False
        target_dir = instructions / "agent-pull-requests"
