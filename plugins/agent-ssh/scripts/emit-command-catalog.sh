...
command_specs='[{"id":"agent-ssh","relativePath":"bin/agent-ssh","purpose":"Emit inspect and verify SSH fabric profiles"}]'
hook_input=""
[ -t 0 ] || hook_input="$(cat)"
launch_token=""
ancestor_pid="$PPID"
...
if raw_hook_input:
    try:
        ...
        print(
            f"[agent-ssh] invalid sessionStart payload; "
...
if session_id and launch_token:
    ...
    try:
        ...
        marker_path = ""
        print(
            f"[agent-ssh] catalog deduplication unavailable: {error}",
            file=sys.stderr,
        )
