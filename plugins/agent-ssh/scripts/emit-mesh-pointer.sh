...
# `agent-ssh mesh-status` for the full table -- rather than dumping the whole mesh
...
# and robust; all rendering is deferred to the `agent-ssh mesh-status` binstub).
...
(`--summary` for one line, `--json` for structured). Reach a host interactively
with `ssh <alias>` (the aliases are listed by `agent-ssh mesh-status`). Reachability is
**dtssh** -- live only while the target is powered on and logged in; `ssh.ready`
is the operator's declared state, so `agent-ssh verify <alias>` probes a host
live.
MD
