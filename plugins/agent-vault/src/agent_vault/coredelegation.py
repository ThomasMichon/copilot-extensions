# Vendored from libs/core-delegation (the canonical shared module). Kept
# in-package -- not a distribution dependency -- so agent-vault installs as a
# self-contained plugin with no external package to resolve. The ONLY adaptation
# from the canonical source is the discovery import below: `endpoint_rendezvous`
# -> the sibling vendored `.rendezvous` module. Sync changes from the canonical.
