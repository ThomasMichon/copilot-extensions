...
from .remote_launch import build_remote_launch  # noqa: F401 -- compatibility re-export
...
class CodeSpaceSpawner:
    ...
    async def spawn(
        ...
        *,
        ...
        nonce = new_nonce()
        if self.boundary in {"codespace", "container"}:
        ...
        launch = build_remote_launch(
            ...
            unexpected_reap_seconds=self._unexpected_reap_seconds,
            active_reap_seconds=self._active_reap_seconds,
            venue_id=self.boundary if self.boundary in {"codespace", "container"} else "",
        )
