...
command_specs='[{"id":"agent-bridge","relativePath":"bin/agent-bridge","purpose":"Communicate with persistent agent sessions"}]'
hook_input=""
[ -t 0 ] || hook_input="$(cat)"
launch_token=""
ancestor_pid="$PPID"
...
if raw_hook_input:
    try:
        ...
        print(
            f"[agent-bridge] invalid sessionStart payload; "
...
if session_id and launch_token:
    ...
    try:
        ...
        marker_path = ""
        print(
            f"[agent-bridge] catalog deduplication unavailable: {error}",
            file=sys.stderr,
        )
