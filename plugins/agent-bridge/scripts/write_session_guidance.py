#!/usr/bin/env python3
"""Write agent-bridge guidance as a sessionStart side effect.
...
_GUIDANCE_HEADER = "# Agent Bridge session guidance\n\n"
...
def write_session_guidance(payload: dict, *, home: Path | None = None) -> bool:
    ...
    if contexts:
        ...
        content = (
            _GUIDANCE_HEADER
            + "No current agent-bridge session guidance was available. Treat "
            "guidance as unavailable for this session-start invocation.\n"
        )
    if contexts and len(content.encode("utf-8")) > _GUIDANCE_MAX_BYTES:
        content = (
            _GUIDANCE_HEADER
            + "Current agent-bridge session guidance was omitted because it "
    ...
    try:
        ...
        if _is_link_or_reparse(instructions_dir):
            return False
        target_dir = instructions_dir / "agent-bridge"
