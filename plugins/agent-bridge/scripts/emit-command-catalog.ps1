...
$specs = '[{"id":"agent-bridge","relativePath":"bin\\agent-bridge.ps1","purpose":"Communicate with persistent agent sessions"}]' | ConvertFrom-Json
$commands = @(
    ...
    if ($hookInput) {
        try {
            ...
            [Console]::Error.WriteLine(
                '[agent-bridge] invalid sessionStart payload; catalog deduplication disabled: ' +
                $_.Exception.Message
            )
        }
    }
...
try {
    ...
    [Console]::Error.WriteLine(
        '[agent-bridge] catalog deduplication launch identity unavailable: ' +
        $_.Exception.Message
    )
}
...
if ($sessionId -and $launchToken) {
    ...
    try {
        ...
        $markerPath = ''
        [Console]::Error.WriteLine(
            '[agent-bridge] catalog deduplication unavailable: ' +
            $_.Exception.Message
        )
    } catch {
        $markerPath = ''
