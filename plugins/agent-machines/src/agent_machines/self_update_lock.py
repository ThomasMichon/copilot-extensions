"""Tier locking for agent-machines self-update -- one named lock per tier so
two concurrent runs (an interactive invocation racing the scheduled task/timer,
or two scheduled fires overlapping) never step on each other.

Split out of ``self_update.py`` (module-size baseline) since the locking
concern is self-contained: a fast, atomic "did I win the race" primitive
(``_WindowsMutex`` on Windows, ``_PosixFileLock`` via ``flock()`` on
Linux/WSL) backed by a JSON lock-record file used for the already
cross-platform pid/staleness bookkeeping (a dead owner's lock is reclaimed
only once its state has been stale for longer than the tier's own
...
from .self_update_state import STATE_VERSION, TIER_SPECS, lock_path, mutex_name
...
class _PosixFileLock:
    """POSIX equivalent of ``_WindowsMutex``: an ``flock()``-based advisory
    lock on a dedicated file (separate from the JSON lock *record* used for
    the cross-process pid/staleness bookkeeping below -- this only provides
    the atomic "did I win the race" primitive that a named Win32 mutex gives
    for free). ``flock()`` is automatically released if the holding process
    dies or the fd is closed, so a crashed run can never wedge the lock
    permanently the way a stale JSON record alone could.
    """
...
def _posix_lock_file_path(tier: str, home: Path | None = None) -> Path:
    # Distinct from lock_path()'s '<tier>.lock.json' record file: this one is
    # never parsed, only flock()'d.
