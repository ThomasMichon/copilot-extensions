...
def _cmd_fleet_update_run(args: argparse.Namespace) -> int:
...
def _cmd_fleet_update_install(args: argparse.Namespace) -> int:
    resolved_state = _resolve_fleet_update_resources(args)
    if resolved_state is None:
        return 1
    _identity, _machine, selected_resources = resolved_state
...
def _cmd_fleet_update_status(args: argparse.Namespace) -> int:
    resolved_state = _resolve_fleet_update_resources(args)
    if resolved_state is None:
        return 1
    _identity, _machine, selected_resources = resolved_state
...
def _cmd_fleet_update_uninstall(args: argparse.Namespace) -> int:
    results = [
        _fleet_update.reconcile_scheduled_task(
            tier,
            desired_present=False,
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    fleet_update_run.set_defaults(func=_cmd_fleet_update_run)
    for subcommand, func, help_text in (
        (
            "install",
            _cmd_fleet_update_install,
            "Register opted-in fleet-update Scheduled Tasks.",
        ),
        (
            "status",
            _cmd_fleet_update_status,
            "Inspect fleet-update opt-in and Scheduled Task state.",
        ),
        ("uninstall", _cmd_fleet_update_uninstall, "Remove fleet-update Scheduled Tasks."),
    ):
