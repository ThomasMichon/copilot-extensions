...
command_specs='[{"id":"agent-machines","relativePath":"bin/agent-machines","purpose":"Discover validate and restore declared machine state"}]'
hook_input=""
[ -t 0 ] || hook_input="$(cat)"
...
if raw_hook_input:
    try:
        ...
        print(
            f"[agent-machines] invalid sessionStart payload; "
...
if session_id:
    ...
    try:
        ...
        marker_path = ""
        print(
            f"[agent-machines] catalog deduplication unavailable: {error}",
            file=sys.stderr,
        )
