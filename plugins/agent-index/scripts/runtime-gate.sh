...
if [ "${EFFECTIVE_FIELDS[0]:-0}" != 1 ]; then
    if [ "$COMMAND" = installer-readiness ]; then
        printf '%s\n' \
            '{"detail":"No effective .agent-index/config.yaml or repo-local agent-index overlay opts this repository in; session startup remains non-mutating.","module":"agent-index/runtime","schema":"copilot-extensions.module-readiness","state":"configuration-empty","version":1}'
        exit 0
    fi
    _emit_inactive
    [ "$COMMAND" = status ] && exit 0
...
if { [ "$RESOLUTION_STATUS" = ready ] &&
   ...
   [ "$SIMPLE_POLICY_LEGACY" = 1 ]; then
    ...
    cd "$ROOT"
elif [ "$COMMAND" = installer-readiness ] &&
     [ "$RESOLUTION_STATUS" = provenance-blocked ] &&
     [ -z "${COPILOT_EXTENSIONS_CONTEXT:-}" ]; then
    ...
    printf '%s\n' \
        '{"schema":"copilot-extensions.module-readiness","version":1,"module":"agent-index/runtime","state":"configuration-empty","detail":"This client has not completed agent-index setup (no installation-mode policy or active context yet); session startup remains non-mutating."}'
    exit 0
else
    printf '[agent-index] installation context blocks invocation: status=%s reason=%s.\n' \
