...
$specs = '[{"id":"agent-index","relativePath":"bin\\agent-index.ps1","purpose":"Search and operate the semantic index"}]' | ConvertFrom-Json
$commands = @(
    ...
    if ($hookInput) {
        try {
            ...
            [Console]::Error.WriteLine(
                '[agent-index] invalid sessionStart payload; catalog deduplication disabled: ' +
                $_.Exception.Message
            )
        }
    }
...
try {
    ...
    [Console]::Error.WriteLine(
        '[agent-index] catalog deduplication launch identity unavailable: ' +
        $_.Exception.Message
    )
}
...
if ($sessionId -and $launchToken) {
    ...
    try {
        ...
        $markerPath = ''
        [Console]::Error.WriteLine(
            '[agent-index] catalog deduplication unavailable: ' +
            $_.Exception.Message
        )
    } catch {
        $markerPath = ''
