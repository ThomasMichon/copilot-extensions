...
command_specs='[{"id":"agent-index","relativePath":"bin/agent-index","purpose":"Search and operate the semantic index"}]'
hook_input=""
[ -t 0 ] || hook_input="$(cat)"
launch_token=""
ancestor_pid="$PPID"
...
if raw_hook_input:
    try:
        ...
        print(
            f"[agent-index] invalid sessionStart payload; "
...
if session_id and launch_token:
    ...
    try:
        ...
        marker_path = ""
        print(
            f"[agent-index] catalog deduplication unavailable: {error}",
            file=sys.stderr,
        )
