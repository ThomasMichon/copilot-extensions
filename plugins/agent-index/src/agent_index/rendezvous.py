...
# in-package -- not a distribution dependency -- so agent-index installs as a
# self-contained git dependency (some deployments consume it that way) with no
...
class Endpoint:
    ...
    def parse(cls, spec: str, *, source: str = "file") -> Endpoint:
        """Parse a ``"<transport>:<address>"`` spec, e.g. ``"tcp:127.0.0.1:52731"``.
...
def default_runtime_dir(app: str) -> Path:
    """The conventional runtime dir for an app, e.g. ``~/.agent-index/run``."""
