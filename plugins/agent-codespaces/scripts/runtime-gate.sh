...
PAYLOAD_ROOT="${AGENT_CODESPACES_PAYLOAD_ROOT:-}"
[[ "$PAYLOAD_ROOT" == /* && -d "$PAYLOAD_ROOT" ]] || {
    printf '[agent-codespaces] owning payload root is unavailable.\n' >&2
    exit 126
}
...
LEGACY_ROOT="${AGENT_CODESPACES_HOME:-$HOME/.agent-codespaces}" # marketplace-isolation: allow legacy compatibility root
SEP=$'\034'
...
run_runtime() {
    export AGENT_CODESPACES_HOME="$RUNTIME_ROOT"
    export AGENT_CODESPACES_PAYLOAD_ROOT="$PAYLOAD_ROOT"
    if [[ -n "$CONTEXT" ]]; then
        export COPILOT_EXTENSIONS_CONTEXT="$CONTEXT"
    else
        unset COPILOT_EXTENSIONS_CONTEXT
    fi
    exec "$AGENT_RT_PY" -m agent_codespaces "$@"
}

[[ -f "$MODE_RUNNER" && -f "$JSON_QUERY" && -f "$RUNTIME_RESOLVER" &&
   -f "$INSTALLER" ]] || {
    printf '[agent-codespaces] installation-context runtime support is unavailable.\n' >&2
    exit 126
}
...
if [[ "$RESOLUTION_RC" -ne 0 ]]; then
    printf '[agent-codespaces] installation context could not be resolved: %s\n' \
        "$RESOLUTION" >&2
    exit 126
fi
...
if [[ "$RESOLUTION_STATUS" == ready &&
    ...
    if [[ -n "${COPILOT_EXTENSIONS_CONTEXT:-}" ]]; then
        printf '[agent-codespaces] requested installation context is not active.\n' >&2
        exit 126
    fi
    ...
    if [[ -z "$RUNTIME_ROOT" || -z "$CONTEXT" ]]; then
        printf '[agent-codespaces] active installation context is incomplete.\n' >&2
        exit 126
    fi
    ...
    VALIDATION="$(
        bash "$MODE_RUNNER" validate \
            ...
            --expected-plugin-id agent-codespaces \
            --expected-payload-root "$PAYLOAD_ROOT"
    )" || {
        printf '[agent-codespaces] installation context validation failed.\n' >&2
        exit 126
    }
    ...
    [[ "$VALIDATED_INSTALL_GENERATION" == "$INSTALL_GENERATION" ]] || {
        printf '[agent-codespaces] installation context generation does not match governance.\n' >&2
        exit 126
    }
else
    printf '[agent-codespaces] installation context blocks invocation: status=%s reason=%s.\n' \
...
if [[ -n "${AGENT_CODESPACES_NO_SELFPROVISION:-}" ]]; then
    printf '[agent-codespaces] runtime not provisioned (AGENT_CODESPACES_NO_SELFPROVISION set).\n' >&2
    exit 1
fi
...
printf '%s\n' '[agent-codespaces] runtime not provisioned -- provisioning on first use (may take ~30-120s: acquires uv + builds a venv). Do not kill; extend your timeout.' >&2
printf '::agent-provisioning:: plugin=%s eta_seconds=120 reason=first-use status=%s\n' \
    'agent-codespaces' "$STATUS_PATH" >&2
...
if [[ "$PROVISION_EXIT" -ne 0 ]]; then
    ...
    printf '[agent-codespaces] provisioning FAILED. See the log above; retry, or run: bash "%s" provision --install-dir "%s"\n' \
...
printf '[agent-codespaces] provisioning completed without a resolvable runtime.\n' >&2
exit 1
