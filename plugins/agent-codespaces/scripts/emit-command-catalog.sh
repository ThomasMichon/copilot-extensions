...
command_specs='[{"id":"agent-codespaces","relativePath":"bin/agent-codespaces","purpose":"Manage GitHub Codespaces lifecycle transport and dispatch"}]'
hook_input=""
[ -t 0 ] || hook_input="$(cat)"
launch_token=""
ancestor_pid="$PPID"
...
if raw_hook_input:
    try:
        ...
        print(
            f"[agent-codespaces] invalid sessionStart payload; "
...
if session_id and launch_token:
    ...
    try:
        ...
        marker_path = ""
        print(
            f"[agent-codespaces] catalog deduplication unavailable: {error}",
            file=sys.stderr,
        )
