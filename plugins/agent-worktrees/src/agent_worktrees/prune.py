...
def interpret_descriptor_payload(payload: dict | None) -> dict:
    ...
    if held_claims is None or open_follow_ups is None:
        # A malformed count (not a valid non-negative int) must never be
        # laundered into ``0`` -- that would become false evidence "no
        # blockers exist" for the FINAL invariant just below. Reject the
        # whole payload instead of guessing.
...
def _non_negative_int(value) -> int | None:
    ...
    evidence that no blockers exist (see the FINAL invariant just above).
    The caller rejects the whole payload as unsupported when this returns
    ``None``."""
