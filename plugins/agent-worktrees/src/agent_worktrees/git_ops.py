...
def slug_from_url(url: str | None) -> str | None:
    """Parse the hosting ``owner/name`` slug from a remote *URL*, or None.

    The pure URL-parsing core of :func:`remote_slug` (which resolves a remote
    *name* to its URL first). Parses the last two path components (dropping a
    trailing ``.git``), so it works for both https and ssh forms and for
    self-hosted hosts with a path prefix, and yields the **provider-correct**
    slug for Azure DevOps (``{project}/{repo}``), e.g.:
    ...
    as opposed to the local project name. Accepts a URL string (callers that
    have only a remote *name* should use :func:`remote_slug`).
    """
    if not url:
        return None
    s = url.strip().rstrip("/")
...
def remote_slug(remote: str, *, cwd: str | Path) -> str | None:
    """Return the hosting ``owner/name`` slug for the remote *named* ``remote``.

    Resolves the remote name (e.g. ``origin``) to its configured URL in ``cwd``,
    then defers to :func:`slug_from_url` for the provider-correct parse. Callers
    that already hold a remote URL should call :func:`slug_from_url` directly.
    """
