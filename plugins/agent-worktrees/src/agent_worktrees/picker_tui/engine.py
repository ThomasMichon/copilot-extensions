...
# semantic ``style`` token (``prune.assemble_closure_descriptor``/
# ``interpret_descriptor_payload``) mapped to the Picker's native palette --
# the same translation the PSMux/TMux status segment's ``_DESCRIPTOR_STYLE_BG``
# already performs for its terminal-256 palette. A style adapter never
# redefines state; it only chooses colour. An unrecognized/empty token (no
# descriptor, or one an old/new remote emitted that this build doesn't know)
# falls back to the legacy label-keyed ``C_STATE`` lookup in ``row_text``.
_DESCRIPTOR_STYLE = {
    "final": C_STATE["FINAL"],
...
class PickerScreen(Widget):
    ...
    def __init__(
        ...
        # run before the first frame (#1504). ``setup()`` / live async fill scan.
    ...
    def _load_pivots(self, *, scan: bool = True):
        ...
        paint is not blocked on the registry (#1504).
        """
        if not scan:
    ...
    def _setup_skeleton(self):
        """First-paint chrome with no config, roster, git, or pivot-registry I/O.

        Hostname-only local tab; ``setup()`` replaces it with the real roster.
        """
    ...
    def _prepare_live_source(self, snapshot):
        """Resolve all source/config-derived values on the setup worker."""
        source_tabs = getattr(self.src, "source_tabs", None)
        if callable(source_tabs):
            tabs = list(
                source_tabs(snapshot) if snapshot is not None else source_tabs()
            )
        else:
            machine_tabs = (
    ...
    def local_index(self):
        # Do not touch ``src.LOCAL`` until the roster is ready: on the SSH
        # source that resolves machines.yaml via ``load_config`` (#1504).
    ...
    def _confirm_new_worktree(self, dlg):
        ...
        self._decide({
            ...
            "is_local": (tm, te) == self._src_local(),
            "options": {
