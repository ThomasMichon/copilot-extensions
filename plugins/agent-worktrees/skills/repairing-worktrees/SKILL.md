...
each prefix element separately, and never search `PATH`. Cross-plugin
`<agent-dispatch catalog argv prefix>` examples use that plugin's exact catalog
prefix under the same rule.
Project-binstub examples remain project entry points.

`<agent-worktrees catalog argv prefix> doctor` is the single repeatable primitive for worktree/session
...
registering the real session and promoting it over the placeholder:
   ```
   <agent-worktrees catalog argv prefix> register-session --cwd <worktree-path> --session-id <real-id>
   <agent-worktrees catalog argv prefix> link-succession --worktree <id> \
       --predecessor <placeholder-id> --successor <real-id> \
       --predecessor-state concluded          # retire the placeholder, promote the real head
   <agent-worktrees catalog argv prefix> list-sessions --worktree <id>   # verify: real session is head, turns > 0
   ```
...
chronologically-latest one should be head, promote it:
   ```
   <agent-worktrees catalog argv prefix> register-session --cwd <worktree-path> --session-id <missing-id>   # repeat, oldest→newest
   <agent-worktrees catalog argv prefix> link-succession --worktree <id> \
...
- **Best: file a durable close-out *task* and let worktrees resolve async.**
  ...
  ```
  <agent-dispatch catalog argv prefix> create "Close out <id>" \
  ...
  ```
  **Dedup first** (`<agent-dispatch catalog argv prefix> find` / `sweep`) so you file one close-out per
