...
every shell operation below. Replace `<agent-worktrees catalog argv prefix>` with
that prefix with each element quoted for the current shell, and never search `PATH`.
...
one with `<agent-worktrees catalog argv prefix> repos migrate`.
...
cd <target-repo-checkout> && <agent-worktrees catalog argv prefix> register <name>   # cwd = the subject
#   …or, from anywhere, name the path explicitly:
<agent-worktrees catalog argv prefix> register <name> --repo-dir <path>
<agent-worktrees catalog argv prefix> repos add <name> <path> --class worktree       # path-first alt
...
<agent-worktrees catalog argv prefix> create # prints id + path (add --json for a plan)
...
`<agent-worktrees catalog argv prefix> repos <subcommand>`:
...
```bash
<agent-worktrees catalog argv prefix> repos migrate
<agent-worktrees catalog argv prefix> repos list          # verify, then reclassify as needed
```

Reclassify any entry by re-adding it with the right class:

```bash
<agent-worktrees catalog argv prefix> repos add copilot-extensions 'D:\Src\copilot-extensions' --class worktree
```

### Set up source roots

```bash
<agent-worktrees catalog argv prefix> repos srcroot --set 'D:\Src' --platform windows
<agent-worktrees catalog argv prefix> repos srcroot --set ~/src --platform wsl
```

### Register an existing repo

```bash
<agent-worktrees catalog argv prefix> repos add my-lib 'D:\Src\my-lib' --class reference \
...
```bash
<agent-worktrees catalog argv prefix> repos find my-project
# → D:\Src\my-project
```
...
```bash
<agent-worktrees catalog argv prefix> repos status                 # branch, dirty, ahead/behind
<agent-worktrees catalog argv prefix> repos sync --tag multi-machine system    # fetch + ff-merge (skips dirty)
```
...
```
<agent-worktrees catalog argv prefix> accounts set <gh-login> --scopes codespace,repo,workflow \
