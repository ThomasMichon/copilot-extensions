...
Use the exact `argv` prefix from each plugin's session command catalog for the
interactive operations below. Replace `<agent-worktrees catalog argv prefix>`,
`<agent-bridge catalog argv prefix>`, `<agent-codespaces catalog argv prefix>`, and
`<agent-containers catalog argv prefix>` with their raw published paths. Quote each
...
`<agent-worktrees catalog argv prefix> related resolve` give you the plan.

## The one command to start with

```bash
<agent-worktrees catalog argv prefix> related resolve <name>     # or: related resolve   (uses the primary)
<agent-worktrees catalog argv prefix> related resolve <name> --json
```
...
- **The target's `AGENTS.md` is *its* POV; the narrative is *ours*.** The related
  narrative (`<agent-worktrees catalog argv prefix> related doc <name>`) is this control-plane's view
...
- **reference** -- *read-only*. Resolve the path with
  `<agent-worktrees catalog argv prefix> repos find <name>` and read it. **Never edit** a reference
  repo locally.
...
- **worktree** -- never edit the anchor. Create an isolated worktree, edit and
  ...
  tool call, use `<agent-worktrees catalog argv prefix> create` (prints the worktree path; cd in and
  ...
  **not adopted**, adopt it first
  (`<agent-worktrees catalog argv prefix> register <name>`).

Always read the repo's `CONTRIBUTING.md` / `AGENTS.md` and its narrative
(`<agent-worktrees catalog argv prefix> related doc <name>`) before changing it -- orient at the root
...
- **machine:&lt;key&gt;** and it is a **different** machine -- **delegate** to it
  via agent-bridge:
  `<agent-bridge catalog argv prefix> send <key> "<task>"`. Don't clone it locally
  just to avoid delegating.
- **codespace** -- provision/connect via **agent-codespaces** and dispatch via
  agent-bridge:
  `<agent-codespaces catalog argv prefix> create <cs-repo>` (headless, no TTY -- routes around
  ...
  `<agent-codespaces catalog argv prefix> ssh <name>`.
...
> (`<agent-codespaces catalog argv prefix> ssh <name>` /
> `<agent-containers catalog argv prefix> up <name>`; on a fleet host
...
> auto-journaled — only `<agent-worktrees catalog argv prefix> create-pr` in *this* repo is. So your
...
> ```
> <agent-worktrees catalog argv prefix> claims add pr <pr-url> \
>   --owner-ref "$(<agent-worktrees catalog argv prefix> get owner-ref)"
> # when it merges/closes:
> <agent-worktrees catalog argv prefix> claims settle <pr-url>     # (sweep spares pr-kind — manual)
> ```
...
`<agent-worktrees catalog argv prefix> repos find <name>` (it falls back to the per-machine
...
3. Act on the plan:
   - local -> edit per class (worktree
     `<agent-worktrees catalog argv prefix> create` / singleton
...
4. Land changes through the **target repo's** own contribution flow (its branch
   ...
   - **Check the target repo's PR flow before you drive one:**
     `<agent-worktrees catalog argv prefix> get pr-profile` reports `direct` (no PR),
