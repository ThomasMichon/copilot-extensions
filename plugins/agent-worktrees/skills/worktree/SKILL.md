...
> **`register` (adopt) is the exception — cwd is the only locator.** Because a
> project binstub / `--project <name>` resolves an *already-adopted* project,
> those levers don't exist for the repo you're about to adopt. `agent-worktrees
> register <name>` therefore takes the repo **path from cwd** (the git root of
> the current directory → its anchor); `<name>` is only the project **label**.
> So run `register` **from inside the target repo's checkout**, or pass
> `--repo-dir <path>` (or use `repos add <name> <path>`). Running `register <name>`
> from a *different* repo silently adopts *that* repo's path under `<name>`.
