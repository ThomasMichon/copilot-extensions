...
| `head_scheme` | string | `refspec` | How `create-pr` publishes the PR head. `refspec` (default, #1815): keep the squashed work **on** `worktree/<id>` and push it directly to the PR head ref (`worktree/<id>:refs/heads/<head>`) — no local feature branch, no checkout dance; the worktree stays on its own branch (which sits ahead of master while the PR is open). `snapshot` (legacy, opt-in): snapshot the squashed commit onto a separate local `feature/<slug>` branch, reset the worktree base to upstream, push that branch, and return HEAD to the worktree branch (#1804). A parallel `--new` PR auto-falls-back to a snapshot ref even under `refspec`. |
