...
| `head_scheme` | string | `snapshot` | How `create-pr` publishes the PR head. `snapshot` (default, #1804): snapshot the squashed commit onto a separate local `feature/<slug>` branch, reset the worktree base to upstream, push that branch, and return HEAD to the worktree branch. `refspec` (feature-complete, opt-in, #1815): keep the squashed work **on** `worktree/<id>` and push it directly to the PR head ref (`worktree/<id>:refs/heads/<head>`) — no local feature branch, no checkout dance; the worktree stays on its own branch (which sits ahead of master while the PR is open). Refspec stays opt-in until the facility pre-push hook fix (which permits the mediated `worktree/<id> → pr/<slug>` push) is deployed to every PR-mode repo/machine — flipping the default before then breaks `create-pr` on any machine still running the old hook. A parallel `--new` PR auto-falls-back to a snapshot ref even under `refspec`. |
