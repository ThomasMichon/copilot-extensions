...
| `allow_stale_approval` | bool | `false` | Allow the latest approval to authorize `pr-merge` after the PR head changes, provided the live PR is otherwise mergeable and unblocked. The `pr-status` / `pr-watch` payloads report `approval_stale: true`. Enable only when repository policy permits stale-head approval authority. |
