...
| `head_pattern` | string | *(provider/scheme default)* | Template for the PR head branch name. Tokens: `{prefix}` `{slug}` `{suffix}` `{username}` `{machine}`. Empty ⇒ provider-aware default: `azure-devops` uses `user/{username}/{slug}-{suffix}` regardless of `head_scheme`; other providers keep the scheme defaults (`pr/{slug}-{suffix}` under `refspec` and `{prefix}/{slug}-{suffix}` under `snapshot` / `feature/<slug>`). `{username}` resolves from the repo's git identity (`user.email` local-part, then `user.name`). **When `source_attribution` isn't `true`, a `head_pattern` (or an explicit `--branch`, or an existing-PR-reuse) whose effective resolved head embeds `{machine}`, the raw worktree id, or an unresolved `{worktree_id}` marker is a hard `create-pr` error, not a warning** — see `attribution-audit` below.
