...
| `remux` | **Linux/WSL only.** Reparent a running **bare** (un-muxed) Copilot into its `wt-<id>` tmux pane via `reptyr`, so no conversation is lost and the session rejoins the mux fleet -- the companion to `reclaim` (which reaps-and-resumes). Target with `--session-id` / `--worktree-id` (infers from cwd). Runs `reptyr` under `sudo -A` when the kernel `yama` `ptrace_scope` forbids attaching a non-descendant (auto-detected; force with `--sudo` / `--no-sudo`). Verifies via the process's controlling tty (reptyr moves the tty, not the ppid). A clear no-op on Windows (ConPTY cannot adopt a running process) |
...
| `status` | Show worktree git status; **write mode** (`--summary "<one-liner>"` / `--title "<headline>"` / `--follow-up` / `--resolved`) annotates THIS worktree's Picker disposition. A `postToolUse` hook nudges you to refresh it as work drifts (`AGENT_WORKTREES_NUDGE=off` to silence) |
