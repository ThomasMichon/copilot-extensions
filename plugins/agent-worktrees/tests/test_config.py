...
class TestPRConfigParsing:
    ...
    def test_codename_block_wires_through_load_config(self, tmp_path: Path):
        ...
        self._write(
            cfgfile,
            "    codename:\n"
            "      hook_command: my-hook --generate\n"
            "      hook_timeout_seconds: 2.5\n",
        )
        codename = cfg.load_config(cfgfile).repos["ext"].codename
        assert codename.hook_command == "my-hook --generate"
        assert codename.hook_timeout_seconds == 2.5

    def test_codename_absent_defaults_to_no_hook(self, tmp_path: Path):
        cfgfile = tmp_path / "config.yaml"
        self._write(cfgfile)
        codename = cfg.load_config(cfgfile).repos["ext"].codename
        assert codename.hook_command == ""
