...
command_specs='[{"id":"agent-worktrees","relativePath":"bin/payload/agent-worktrees","purpose":"Manage worktrees and project lifecycle"}]'
hook_input=""
[ -t 0 ] || hook_input="$(cat)"
launch_token=""
ancestor_pid="$PPID"
...
if raw_hook_input:
    try:
        ...
        print(
            f"[agent-worktrees] invalid sessionStart payload; "
...
if session_id and launch_token:
    ...
    try:
        ...
        marker_path = ""
        print(
            f"[agent-worktrees] catalog deduplication unavailable: {error}",
            file=sys.stderr,
        )
