...
deploy_wrappers() {
    ...
    # Deploy hook scripts: sessionStart (session-conduct + session-machine + bootstrap-check + project-hooks + register-session + anchor-hygiene-check + provision-check) + preToolUse guards (statelessness_guard + cross_repo_guard + anchor_write_guard) + postToolUse nudge (nudge_status)
