...
$specs = '[{"id":"agent-worktrees","relativePath":"bin\\payload\\agent-worktrees.ps1","purpose":"Manage worktrees and project lifecycle"}]' | ConvertFrom-Json
$commands = @(
    ...
    if ($hookInput) {
        try {
            ...
            [Console]::Error.WriteLine(
                '[agent-worktrees] invalid sessionStart payload; catalog deduplication disabled: ' +
                $_.Exception.Message
            )
        }
    }
...
try {
    ...
    [Console]::Error.WriteLine(
        '[agent-worktrees] catalog deduplication launch identity unavailable: ' +
        $_.Exception.Message
    )
}
...
if ($sessionId -and $launchToken) {
    ...
    try {
        ...
        $markerPath = ''
        [Console]::Error.WriteLine(
            '[agent-worktrees] catalog deduplication unavailable: ' +
            $_.Exception.Message
        )
    } catch {
        $markerPath = ''
