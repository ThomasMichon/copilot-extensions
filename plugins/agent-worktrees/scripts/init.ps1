<#
.SYNOPSIS
    Bootstrap the agent-worktrees shared runtime (delegates to install.ps1).
...
.PARAMETER InstallDir
    Accepted for compatibility; ignored. The runtime is always ~/.agent-worktrees.
...
if ($InstallDir) {
    Write-Warning "init.ps1 no longer supports -InstallDir; using ~/.agent-worktrees."
}
# Shared-runtime bootstrap only: never auto-adopt a project.
$env:WORKTREE_PROJECT = $null
