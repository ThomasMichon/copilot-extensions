...
command_specs='[{"id":"agent-mcp","relativePath":"bin/agent-mcp","purpose":"Wrap authenticate and materialize MCP servers"}]'
hook_input=""
[ -t 0 ] || hook_input="$(cat)"
...
if raw_hook_input:
    try:
        ...
        print(
            f"[agent-mcp] invalid sessionStart payload; "
...
if session_id:
    ...
    try:
        ...
        marker_path = ""
        print(
            f"[agent-mcp] catalog deduplication unavailable: {error}",
            file=sys.stderr,
        )
