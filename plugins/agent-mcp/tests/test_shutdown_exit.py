...
from agent_mcp import _shutdown_exit
...
def test_console_entry_wraps_main() -> None:
    ...
    ``python -m agent_mcp`` -- the generated console-script wrapper
    ...
    """
    from agent_mcp import __main__ as m
...
def test_installed_console_script_maps_to_console_entry() -> None:
    ...
    back to ``agent_mcp.__main__:main`` -- the generated
    ``agent-mcp`` executable calls whatever that mapping names, not
    whatever this test module imports. Parse the actual mapping instead.
    """
    pyproject_path = Path(__file__).resolve().parents[1] / "pyproject.toml"
    data = tomllib.loads(pyproject_path.read_text(encoding="utf-8"))
    scripts = data["project"]["scripts"]
    assert scripts["agent-mcp"] == "agent_mcp.__main__:console_entry"
