---
name: ado-defer
description: "ADO read-only (defer variant). Catalog hidden behind find_tool/execute_tool. Enumerate PRs, work items in OneDrive/ODSP-Web. No writes."
tools: ["*"]
mcp-servers:
  ado-remote-mcp:
    type: stdio
...
2. `execute_tool` `{tool:"<name>", arguments:{...}}` to run it, e.g.
   `{tool:"repo_pull_request", arguments:{action:"list", project:"ODSP-Web", repositoryId:"odsp-web"}}`.
