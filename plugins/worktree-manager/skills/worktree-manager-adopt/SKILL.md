...
description: >
  ...
  after worktree-manager-init. Trigger phrases include:
  - 'adopt this repo'
  - 'adopt repo'
  - 'register project'
  - 'worktree-manager adopt'
  - 'set up worktree sessions for this repo'
  - 'configure worktree manager for this project'
---
...
**Prerequisite:** the worktree-manager runtime must be installed first
(see the `worktree-manager-init` skill).
...
separately via `worktree-manager services`.
...
worktree-manager status   # should show the adopted repo
```
...
shared runtime at `~/.worktree-manager/` serves all adopted projects.
Run `worktree-manager-adopt` once per repo.
