    ...
    if project:
        ...
        if platform.system() == "Windows":
            ...
            for tool_cmd in ("cleanup", "mark-complete"):
                tool_content = (
                    ...
                    '    exit /b 1\r\n'
                    ')\r\n'
                    f'set "PYTHON=%USERPROFILE%\\.worktree-manager\\.venv\\Scripts\\python.exe"\r\n'
                    f'set "PYTHONPATH=%USERPROFILE%\\.worktree-manager\\lib"\r\n'
                    f'"%PYTHON%" -m worktree_manager {tool_cmd} %*\r\n'
                    "exit /b %ERRORLEVEL%\r\n"
                )
        ...
        for tool_cmd, tool_name in [("cleanup", "cleanup-worktrees"), ("mark-complete", "mark-worktree-complete")]:
            tool_content = (
                ...
                'unset PYTHONHOME\n'
                f'exec "$PYTHON" -m worktree_manager {tool_cmd} "$@"\n'
            )
            dst = lb / tool_name
            dst.write_text(tool_content)
            dst.chmod(0o755)
