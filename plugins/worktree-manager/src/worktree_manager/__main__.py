"""CLI entry point — subcommand dispatcher for worktree-manager.
...
Usage (direct):
    worktree-manager resolve [--dry-run] [--recovery] [--no-mux] [-- args...]
    worktree-manager resolve --json --worktree-id <id>
    worktree-manager list [--json] [--tracking-status active|complete|...]
    worktree-manager create [--json]
    worktree-manager finalize [worktree-id] [--dry-run] [--json]
    worktree-manager mark-complete [worktree-id] [--title T] [--title-only]
    worktree-manager status [--json]
    worktree-manager cleanup [--clean] [--include-unused] [--max-age-days N]
    worktree-manager validate [--dry-run] [--files F...]
    worktree-manager install [--force] [--machine NAME]
    worktree-manager uninstall [--remove-config]
    worktree-manager update
    worktree-manager install-status
    worktree-manager get <key>
    worktree-manager services list [--json]
    worktree-manager services status [--json]
    worktree-manager services check-stale <install_dir> <repo_dir>
    worktree-manager pre-launch
...
The ``worktree-manager`` prefix is stripped for SSH compatibility
(``<project> worktree-manager cleanup`` still works).
"""
...
def cmd_launch(argv: list[str]) -> int:
    ...
    if not launch_script.exists():
        ...
        output.err("Run 'worktree-manager install' first.")
        return 1
...
def cmd_resolve(args: argparse.Namespace) -> int:
    ...
    with output.stdout_to_stderr():
        ...
        if args.copilot_args and args.copilot_args[0] == "wsl":
            ...
            if args.dry_run:
                output.dry_run(f"Would delegate to WSL: WORKTREE_PROJECT={project} ~/.worktree-manager/bin/launch-session.sh {' '.join(remaining)}")
                _emit_plan({"action": "none", "exit_code": 0})
                return 0
            wsl_cmd = ["wsl", "bash", "-lc", f"{no_mux_export}export WORKTREE_PROJECT={project}; ~/.worktree-manager/bin/launch-session.sh {' '.join(remaining)}"]
...
def _post_exit_gate(record: tracking.WorktreeRecord, config: cfg.Config) -> int:
    ...
    if record.status == "complete":
        ...
        output.err(
            ...
            f"Run 'worktree-manager finalize' to retry."
        )
        return 1
...
def cmd_mark_complete(args: argparse.Namespace) -> int:
    ...
    if not success:
        output.warn(
            ...
            "mark-complete or via 'worktree-manager finalize'."
        )
...
def cmd_install(args: argparse.Namespace) -> int:
    ...
    project = cfg.project_name()
    output.header("Installing Worktree Manager")
    ...
    runtime_dir = cfg._home() / ".worktree-manager"
...
def cmd_uninstall(args: argparse.Namespace) -> int:
    output.header("Uninstalling Worktree Manager")
...
def cmd_update(args: argparse.Namespace) -> int:
    project = cfg.project_name()
    output.header("Updating Worktree Manager")
    ...
    if recreate_venv:
        ...
        if not inst.upgrade_venv_deps():
            if inst.is_running_from_managed_venv():
                ...
                output.err("Run: python -m worktree_manager update --recreate-venv")
...
_BOOTSTRAP_SERVICES = ("worktree-manager", "vault")
...
def cmd_pre_launch(args: argparse.Namespace) -> int:
    ...
    # Direct fallback for worktree-manager: always deployed at a known
    # location, but may be missing from service.yaml for this environment
    if "worktree-manager" not in bootstrap:
        ...
        if wm_manifest.exists():
            ...
            if staleness != "current":
                ...
                search_dirs = [Path("services/worktree-manager")]
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Strip `worktree-manager` prefix (SSH compat:
    #   `<project> worktree-manager cleanup` → `cleanup`)
    if args_list and args_list[0] == "worktree-manager":
