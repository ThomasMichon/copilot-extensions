...
|------:|---------:|------|-------|
| 26,191 | +25,191 | `plugins/agent-worktrees/src/agent_worktrees/__main__.py` | First dedicated slice landed: context/services/repos/related dispatch extracted into sibling `*_cli.py` modules; still the worst offender, with the live session/status/PR/install/reap surfaces left for later dedicated slices |
...
- [ ] Work down `tools/rank-module-size.py`'s ranked list, prioritizing
      ...
      - [ ] The `__main__.py` CLI-registration giants
            (`agent-worktrees` 26,191 after its first slice; `agent-bridge`
...
- Validation followed the componentization runbook's stricter rules. Ruff
  ...
  for `agent-worktrees/__main__.py`; the required `check-module-size.py`,
  `check-install-contract.py`, and `check-version-consistency.py` all pass
  afterward. Bumped `agent-worktrees` to `1.5.5-dev200` and
  `.github/plugin/marketplace.json` `metadata.version` to `1.7.7-dev172`.
