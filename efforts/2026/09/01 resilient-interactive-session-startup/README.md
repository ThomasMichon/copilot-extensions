...
- [x] Deferred to `#1548`: Add a deterministic PSMux launch seam that can simulate transient failure, partial-session residue, eventual success, and exhaustion.
- [x] Capture the current sequence from worktree creation through launcher exit and identify which layer owns each state transition.
- [x] Specify retryable versus terminal failure classification, attempt count, delay policy, cancellation behavior, and activity-log fields.
- [x] Specify the preserved-worktree recovery message and machine-readable failure shape.

### Phase 2 - Harden standalone mux startup
- [x] Add bounded retry around PSMux `new-session`, with ownership-safe cleanup before each retry.
- [x] Emit visible attempt/backoff status and structured activity records without duplicating the final failure event.
- [x] On exhaustion, keep an interactive launcher alive for explicit retry or cancel; never prompt a non-interactive caller.
- [x] Ensure cancellation/exhaustion leaves the worktree valid, discoverable, and accurately classified rather than silently deleting it.
- [x] Apply equivalent failure semantics to tmux where practical, while keeping backend-specific retry classification.

### Phase 3 - Expose a composable launch transaction
- [x] Deferred to `#1548`: Separate create, mux-start, readiness, attach, and recovery outcomes behind a machine-readable agent-worktrees contract.
- [x] Deferred to `#1548`: Preserve `create --json` as mux-independent and preserve direct `resolve --new` without requiring the Picker.
- [x] Deferred to `#1548`: Make repeated launch requests idempotently join a ready session or retry a recoverable failed launch without creating another worktree.
- [x] Deferred to `#1548`: Document the contract boundary: agent-worktrees owns mechanics and state; presentation layers own interactive progress and choices.

### Phase 4 - Move the new-worktree startup UX into the Picker
- [x] Deferred to `#1548`: Keep the Picker visible while the launch transaction creates the worktree and starts the mux.
- [x] Deferred to `#1548`: Render attempt progress, retry/cancel choices, and the preserved-worktree recovery path in the Picker.
- [x] Deferred to `#1548`: Dismiss/cut over only after the mux reports ready; on failure or cancel, return to the fleet view with the new worktree visible.
- [x] Deferred to `#1548`: Cover local, nested-mux, SSH/ConPTY fallback, and cross-environment launch paths.
- [x] Deferred to `#1548`: Retire duplicated prompt/progress UX from direct launcher adapters once parity is proven, without removing their standalone behavior.
...
- [x] Linux/tmux behavior either matches the contract or documents a tested backend-specific exception.
...
### 2026-09-01 - Standalone recovery implemented
- Added three-attempt mux creation loops on Windows and POSIX, with safe cleanup
  between attempts and an explicit interactive retry after exhaustion.
- Preserved worktrees now report their path and exact reopen command; activity
  records carry attempt count and `recoverable=true`.
- Added focused launcher contract coverage and documented the recovery behavior.
- Moved the structured launch transaction and Picker-owned cutover to #1548.
