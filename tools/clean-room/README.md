...
| [`agent-dispatch-cutover`](scenarios/agent-dispatch-cutover/) | P | **Graceful daemon cutover** (correct-install-flows, dotfiles#1393): install + provision agent-dispatch, then drive the installer-internal `_cutover` seam (the OS-agnostic zdd active/passive cutover) through THORNY situations and assert *a version update never kills in-flight work* — a queued task survives; a claimed+started **held task** survives and the worker re-adopts via the durable queue DB; an **aborted cutover** that strands a drained survivor is healed (undrain); a **wedged daemon** holding an open FD is stood-up-beside and retired (the versioned beside-not-in-place model). Heavy orchestration lives in a portable stdlib-only probe (`fixtures/cutover_probe.py`), verifiable off-Docker too. |
