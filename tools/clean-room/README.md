...
| [`progressive-context-disclosure-eval`](scenarios/progressive-context-disclosure-eval/) | E | **One progressive-disclosure calibration cell:** materializes fresh random 192-bit guide canaries, readable repository/payload guides, and a synthetic session-start plugin outside the read-only fixture mount; drives the declared task through ACP; records eager loading separately; and writes schema-valid zero-turn `INVALID` evidence for setup or transport jams. |
