#!/usr/bin/env bash
# agent-index-cutover/scenario.sh -- Tier-P F1 cutover-resilience scenario.
#
# Validates the BINDING INVARIANT of the correct-install-flows effort
# (dotfiles#1393) for the agent-index SERVICE: a version cutover must NEVER kill
# in-flight, non-resumable work. agent-index's durable unit is its SQLite indexing
# queue ($AGENT_INDEX_HOME/data/tasks.db); the zdd graceful cutover (the real
# `agent-index deploy` seam) stands a new service slot up beside the old, health-
# gates it, flips the routing table (active.json), drains the old, and retires it
# -- so a queued index batch survives and the new service re-adopts it via the
# shared durable DB. The engine daemon (separate unit, fixed port 8421) is left
# untouched by a service cutover and is NOT exercised here.
#
...
#
# Name-free / public F1. Asserts on service/queue OUTCOMES, not exact CLI spelling.
...
CHECKS="${CR_CUTOVER_CHECKS:-survive-queued-batch,drain-gate,breadcrumb-recover}"
PLUGIN="agent-index"
INSTALLED_ROOT="$HOME/.copilot/installed-plugins/$MARKETPLACE_NAME"

: "${CR_SCENARIO_NAME:=agent-index-cutover}"
export CR_SCENARIO_NAME
cr_init
cr_meta "plugin" "$PLUGIN"
cr_meta "validates" "graceful-daemon-cutover (no in-flight index work killed on update)"
...
# Resolve the built versioned-slot python (the service runs from the immutable
# slot, not the .venv link). Prefer the current-version marker; else newest slot.
_resolve_slot_python() {
    local root="$HOME/.agent-index" ver="" p=""
...
_apply_uv_index_fixture
# agent-index needs a HOST role to run a local service (a client runs none).
mkdir -p "$HOME/.agent-index"
printf 'role: host\n' > "$HOME/.agent-index/config.yaml"
export AGENT_INDEX_ROLE=host
...
sleep 8
# First call to the self-provisioning binstub builds the service venv on demand if
# the session-start stamp deferred it.
# First call to the self-provisioning binstub builds the service venv on demand if
# the session-start stamp deferred it. Trigger it in a LOGIN shell so ~/.local/bin
...
capture "provision" -- bash -lc '
  command -v agent-index >/dev/null 2>&1 && agent-index --help >/dev/null 2>&1
  bash "'"$INSTALLED_ROOT/$PLUGIN"'/scripts/install.sh" provision 2>&1 || true
' || true
SLOT_PY="$(_resolve_slot_python || true)"
if [ -n "$SLOT_PY" ] && "$SLOT_PY" -c 'import agent_index; from zdd.cutover import CutoverOrchestrator' >/dev/null 2>&1; then
    ...
    if [ -z "$UV_INDEX" ] && grep -qiE 'HandshakeFailure|pythonhosted|SSL|TLS|certificate' "$CR_LOGDIR"/*.log 2>/dev/null; then
        ...
        jam "path-binstub" "agent-index service runtime/venv (with zdd) NOT provisioned after first session" "cutover battery cannot run without a built slot"
    fi
fi
...
if [ -z "${SLOT_PY:-}" ]; then
    jam "index-cutover" "no slot python -- cannot run the cutover probe" "fix provisioning (phase 2) first"
else
    ...
    while IFS= read -r line; do
        ...
        if [ "$_stat" = "PASS" ]; then
            ...
            jam "index-cutover" "cutover/$_name FAILED: $_rest" "graceful cutover must not kill in-flight index work; see cr-logs/cutover-probe.log"
        fi
    done < <(grep '^PROBE: ' "$_log" 2>/dev/null)
    if [ "$_seen" -eq 0 ]; then
        jam "index-cutover" "cutover probe emitted no PROBE lines (crash before assertions)" "see cr-logs/cutover-probe.log"
    fi
