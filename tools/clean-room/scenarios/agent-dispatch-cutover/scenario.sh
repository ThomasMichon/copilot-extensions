#!/usr/bin/env bash
# agent-dispatch-cutover/scenario.sh -- Tier-P F1 cutover-resilience scenario.
#
# Validates the BINDING INVARIANT of the correct-install-flows effort
# (dotfiles#1393): a version cutover must NEVER kill in-flight, non-resumable
# work. The zdd graceful-cutover mechanism is app-level + OS-agnostic (the same
# Python runs on Linux and Windows), so this Linux clean-room exercises the real
# thing end-to-end -- a real coordinator, real routing table, real durable queue
# DB -- driving the installer-internal `agent-dispatch _cutover` seam through a
# battery of THORNY situations (a queued task, a claimed+started held task, an
# aborted cutover that strands a drained survivor, and a wedged daemon holding an
# open FD in its slot). The heavy orchestration lives in a portable stdlib-only
# probe (fixtures/cutover_probe.py) so it is verifiable off-Docker too.
#
# Name-free / public F1. Asserts on daemon/queue OUTCOMES, not exact CLI spelling.
...
CHECKS="${CR_CUTOVER_CHECKS:-survive-inflight,held-task-readopt,breadcrumb-recover,two-slot-wedged-fd}"
PLUGIN="agent-dispatch"
INSTALLED_ROOT="$HOME/.copilot/installed-plugins/$MARKETPLACE_NAME"

: "${CR_SCENARIO_NAME:=agent-dispatch-cutover}"
export CR_SCENARIO_NAME
cr_init
cr_meta "plugin" "$PLUGIN"
cr_meta "validates" "graceful-daemon-cutover (no in-flight work killed on update)"
...
if [ -n "$SLOT_PY" ] && "$SLOT_PY" -c 'import agent_dispatch; from zdd.cutover import CutoverOrchestrator' >/dev/null 2>&1; then
    ...
    if [ -z "$UV_INDEX" ] && grep -qiE 'HandshakeFailure|pythonhosted|SSL|TLS|certificate' "$CR_LOGDIR"/*.log 2>/dev/null; then
        ...
        jam "path-binstub" "agent-dispatch runtime/venv (with zdd) NOT provisioned after first session" "cutover battery cannot run without a built slot"
    fi
fi
...
if [ -z "${SLOT_PY:-}" ]; then
    jam "dispatch-cutover" "no slot python -- cannot run the cutover probe" "fix provisioning (phase 2) first"
else
    ...
    while IFS= read -r line; do
        ...
        if [ "$_stat" = "PASS" ]; then
            ...
            jam "dispatch-cutover" "cutover/$_name FAILED: $_rest" "graceful cutover must not kill in-flight work; see cr-logs/cutover-probe.log"
        fi
    done < <(grep '^PROBE: ' "$_log" 2>/dev/null)
    if [ "$_seen" -eq 0 ]; then
        jam "dispatch-cutover" "cutover probe emitted no PROBE lines (crash before assertions)" "see cr-logs/cutover-probe.log"
    fi
