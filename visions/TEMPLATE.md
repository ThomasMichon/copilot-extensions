<!--
  ...
  headings stable; agents and humans navigate by them. This repo's addendum
  (visions/README.md § Local conventions) may rename or add sections — follow it
  where it differs from this default.
  ...
  be." It is:
    - INTENT-LEVEL, not a specification. State WHAT should be true; leave HOW to
      the agents/efforts that realize it. Do not pin APIs, schemas, ports, file
      layouts, or step-by-step mechanics here.
...
<!-- The north star. What this subject is FOR, why it exists, and what success
     ultimately looks like. One to a few paragraphs. -->

## Concepts & Components

<!-- The high-level mental model: the major concepts and components the subject
     is composed of, and how they relate. The map, not the blueprint. For a
     BRANCH vision, largely a description of (and links to) child visions.
     DECOMPOSE LIBERALLY: push a component that is its own subject down into a
     child (leaf) vision and link it here with a one-line summary. -->
...
<!-- How the subject should BEHAVE: semantics, invariants, UX expectations,
     failure-mode expectations, performance/latency intent. Enumerated with
     stable headings; intent-level, not a spec. -->
...
<!-- What this subject is deliberately NOT, and where its edges are. -->

## See Also

<!-- Navigation only (NOT a gap list). -->
...
<!--
  OPTIONAL: Provenance / Journal (delete this whole block if unused). A vision
  MAY carry a "## Provenance" section: a non-authoritative revision/derivation
  trail of the vision ITSELF. HARD RULE — it is EXCLUDED from delta derivation
  and records the vision's own history, never the subject's implementation
  status.
-->
