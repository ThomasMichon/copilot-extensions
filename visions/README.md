...
| [venue-pivots-ux](venue-pivots-ux/README.md) | leaf | The **Codespaces and Containers pivots** — a shared two-line row grammar (identity/status/key-status/claims, then a description), repo-first Codespaces identity with a driving-worktree and agent-bridge live-session cross-link, a fleet-vs-general-container split for Containers, and Open/New-venue flows into a muxed Copilot session over SSH (child of agent-fabric, sibling of picker). |
