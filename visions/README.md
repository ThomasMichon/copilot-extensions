...
| [plugins/agent-worktrees](plugins/agent-worktrees/README.md) | leaf | The **isolation & session ground layer's tracking authority** — worktree + session state as a single-owner **live store**, live tracking derived from agent-worktrees' own transports (files/SSH/mux/PowerShell/hooks) with polling as the always-on backbone, an optional losable warm-cache accelerator, and agent-bridge as an *optional* ACP tool/message eventing supplement (child of agent-fabric). |
