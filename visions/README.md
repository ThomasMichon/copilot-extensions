...
| [installer](installer/README.md) | leaf | The **Installer & Configurator** — the standalone, out-of-plugin app that bootstraps a bare machine into a working harness (one-line bootstrap → prereqs → core install → first harness repo) and remains the non-agentic surface for doctoring, config, plugin-prerequisite validation, repo discovery, and Git-referenced presets. |
