...
| [remote-interactive-sessions](remote-interactive-sessions/README.md) | leaf | **Remote interactive sessions** — a human-attended, muxed Copilot CLI session in a remote venue is a first-class peer of a local one: an explicit, per-request CLI mode of the existing Session Host, bound through cwd-keyed discovery rather than ambient self-registration, launched symmetrically by any venue provider over the venue-parity transport (child of agent-fabric). |
