...
| [process-telemetry](process-telemetry/README.md) | leaf | **Process telemetry** — an opt-in, safe-by-default emission hook `agent-*` runtime plugins call around their own process spawns, carrying low-cardinality provenance (plugin, verb, source tag, parent lineage) and best-effort resource figures, so a control-plane can attribute fleet activity back to the fabric that caused it. |
