...
| **agent-dispatch** | coordinator (`serve`, FastAPI); supervisor (`supervise` spawn loop); spawned embodied/headless workers; detached `run` waiters | ⚠️ **Partial** — the coordinator already vendors `zdd` (routing/cutover/breadcrumb/self-retire) and now also has a live self-update loop (opt-in, `AGENT_DISPATCH_SELF_UPDATE=1`) that notices a newer `current-version` slot and self-triggers a cutover; a public `deploy` verb exists (parity with agent-bridge's) for the same cutover run by hand. `install.ps1 update` itself is **not yet** wired to hand off via `-ZeroDowntime` (`zeroDowntimeUpdate` is unset in `plugin.json`), so a plain `copilot plugin update` + reconcile-at-launch still does a kill-and-reinstall unless the self-update loop or a manual `deploy` beats it to the punch | coordinator: between task **claims** (drain = stop claiming, let a claimed-but-unstarted task settle to a resumable state in the queue DB); supervisor: between spawn reservations | Wire `install.ps1 update -ZeroDowntime` (set `"zeroDowntimeUpdate": true`) so the reconcile-at-launch path also hands off via cutover instead of kill-and-reinstall. **Repossess** the supervisor + spawned workers: let them **outlive** the coordinator swap and re-adopt via the durable SQLite queue DB + `running-version.json` (they already run detached). Add `stamp`/`provision` (Thread A) at the same time |
