...
| `~/.agent-worktrees/pivots/*.json` | Malformed manifests do not break the Picker; missing manifests self-restore from installed payloads. | Do not restore disabled contributors; validate command targets; warn without breaking the Picker; expose findings through agent-worktrees doctor. |
| `~/.{project}/config.d/*.yaml` | Bad YAML degrades to an empty fragment and never breaks config loading. | Preserve per-file findings instead of silent `{}`; distinguish managed vs operator fragments; report exact cleanup guidance in project doctor. |
