...
| [entity-relationship-model](entity-relationship-model.md) | The suite's ten durable entity types (worktree, session, task, machine, agent, repo, project, bridge, container, codespace), their owning plugin tier, and a diagnostic playbook mapping each cross-entity traversal question to its current CLI command — or its tracked gap |
