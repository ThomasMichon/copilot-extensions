...
| [graceful-daemon-cutover](graceful-daemon-cutover.md) | How a long-lived local service updates its version **without killing in-flight, non-resumable work**: the shared `zdd` active/passive cutover primitive (routing-table flip + drain at a safe cutover point + breadcrumb recovery), the consumer contract each daemon implements, and per-plugin adoption (agent-bridge reference; agent-index service+engine; agent-dispatch repossession; agent-vault connection-owner) |
