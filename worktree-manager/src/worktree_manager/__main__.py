...
def _cmd_mux_daemon(rest: list[str]) -> int:
    """Manager mux-companion daemon internals + Step-3 launch-path bundle."""
    from . import mux_daemon_cli

    return mux_daemon_cli.cmd_mux_daemon(rest)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args and args[0] in ("--help", "-h"):
        ...
        print("  mux-daemon <run|ensure|register|remove|show|activate|deactivate>")
        print("                         Manager mux-companion daemon internals (Phase 3b Sub-slice 3;")
        print("                         activate/deactivate are called by launch-session.ps1/.sh for")
        print("                         Manager-owned sessions -- the other verbs remain internal)")
        print()
